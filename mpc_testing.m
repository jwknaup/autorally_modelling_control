...
%     [control, xs, us] = LTIMPC(xs{1}, control, x_target, ddd);
    [control, xs, us] = LTVMPC(xs, us, x_target, ddd);
    if isnan(control)
        break
    end
...
for ii = 1:8
    ...
    if ii < 7
        ...
        plot(time(:,1:end-tend), sim_controls(ii-6, 1:end-tend));
    end
end
...
%     Q(3,3) = 10;
    ...
    R(1,1) = 100;
    R(2,2) = 0.00001;
    [A_0, B_0, d_0] = linearize_dynamics(xs{1}, us{1}, ddd);
    A = cell(12,1);
    B = cell(12,1);
    d = cell(12,1);
    for ii = 2:12
        [A{ii-1}, B{ii-1}, d{ii-1}] = linearize_dynamics(xs{ii}, us{ii}, ddd);
    end
    [A{12}, B{12}, d{12}] = linearize_dynamics(xs{13}, us{12}, ddd);
    params.A_0 = A_0;
%     A = zeros(8);
...
function [control, xs, us] = LTIMPC(x_0, u_0, x_target, ddd)
    Q = zeros(8);
    Q(1,1) = 100;
%     Q(2,2) = 1;
%     Q(3,3) = 0.1;
...
%     x_target = [5;0;0;50;50;0;0;0];
    ...
    % u_0 = [0; 30];
    [A, B, d] = linearize_dynamics(x_0, u_0, ddd);
    params.A_0 = A;
    A0 = eye(8);
    A0 = A;
