...
def single_track_cartesian_AJUKF(states, inputs, dt):
    starti = 2000
    window_seconds = 70
    window = window_seconds / dt
    vxj = states[0, starti:window + starti ].T
    vyj = states[1, starti:window + starti ].T
    wzj = states[2, starti:window + starti ].T
    wFj = states[3, starti:window + starti ].T
    wRj = states[4, starti:window + starti ].T
    Yawj = states[5, starti:window + starti ].T
    Xj = states[6, starti:window + starti ].T
    Yj = states[7, starti:window + starti ].T
    deltaj = inputs[0, starti:window + starti ].T
    # Tj = inputs(2, starti:window + starti ].T
    ...
    para0 = np.array([21.75, 1.12, 0.34, 0.23, 0.12, 4, 1.0, 1.0, 1.0])
    para_lb = np.array([15,  0.9, 0.3,  0.2, 0.1, 0.5, 0.5, 0.1, 0.5])
    para_ub = np.array([30,  1.3, 0.4,  0.3, 0.2, 12., 2.5, 2.0, 1.5])
    ...
    Wmi = 1 / (L + lam) / 2
    Wci = Wmi
    sigma_mean_weights = np.vstack((Wm0, (np.ones(2 * L, 1) * Wmi)))
    sigma_cov_weights = np.vstack((Wc0, np.ones(2 * L, 1) * Wci))

    Y = [vxj, vyj, wzj, wFj, Yawj, Xj, Yj]

    state0 = Y[0,:]
    x0 = np.vstack((state0.T, para0.T))
    q0 = np.zeros((num_states_and_params, 1))
    r0 = np.zeros((num_meas, 1))

    init_state_cov = np.diag((P10 * np.ones((1, num_states)))**2)
    init_param_cov = np.diag((P20 * np.ones((1, num_params)))**2)
    P0 = sp.linalg.block_diag((init_state_cov, init_param_cov))

    Qw1 = np.diag((w10 * np.ones((1, num_states)))**2) # state noise
    Qw2 = np.diag((w20**2 * np.ones((1, num_params)))) # param noise
    Q0 = sp.linalg.block_diag((Qw1, Qw2))
    R0 = np.diag(v0**2 * np.ones((1, num_meas)))

    Xa = np.zeros((lth, num_total))
    Xa[0,:] = np.hstack((x0, q0, r0))
    augmented_covariance = blkdiag(P0, Q0, R0)

    Y_opt = np.zeros((lth, num_meas))
    observation_noise_samples = np.zeros((num_meas, lth))
    gammas = np.zeros((num_meas, num_meas, lth))
    observation_noise_means = np.zeros((num_meas, lth))
    observation_noise_covariances = np.zeros((num_meas, num_meas, lth))
    process_noise_samples = np.zeros((num_states_and_params, lth))
    pies = np.zeros((num_states_and_params, num_states_and_params, lth))
    process_noise_means = np.zeros((num_states_and_params, lth))
    process_noise_covariances = np.zeros((num_states_and_params, num_states_and_params, lth))

    # # state estimate
    for k in range(N - 1):

        # # calc sigma predict
        root_Pa = (L + lam) ** 0.5 * np.real(sp.linalg.sqrtm(augmented_covariance))

        if not np.isreal(root_Pa):
            print('Pa not real \n')

        sigma_augmented_prev = np.hstack((Xa[k,:].T, np.repeat(Xa[k,:], (L,1)).T+root_Pa, np.repeat(Xa[k,:], (L, 1)).T-root_Pa))

        sigma_state_params_prev = sigma_augmented_prev[:num_states_and_params,:]
        sigma_process_noise_prev = sigma_augmented_prev[num_states_and_params:2 * num_states_and_params,:]
        sigma_observation_noise_prev = sigma_augmented_prev[2 * num_states_and_params:num_total,:]
        sigma_params_prev = sigma_state_params_prev[num_states:num_states_and_params,:] * scales

        sigma_states_prev = sigma_state_params_prev[:num_states,:]
        control = np.array([deltaj[k], wRj[k], wFj[k]])
        next_state = predict_next_state(sigma_states_prev, control, sigma_params_prev, dt)
        fundisturbed_sigma_states_params_new = np.hstack((next_state, sigma_state_params_prev[num_states:num_states_and_params,:]))
        sigma_states_params_new = fundisturbed_sigma_states_params_new + sigma_process_noise_prev * np.sqrt(dt)

        mean_states_params_new = np.dot(sigma_states_params_new, sigma_mean_weights)

        covariance_states_params_new = np.dot((sigma_cov_weights.T * (sigma_states_params_new - mean_states_params_new)), (sigma_states_params_new - mean_states_params_new).T)
        ...
        hundisturbed_measurements_sigma = sigma_states_params_new[:num_states,:]
        measurements_sigma = hundisturbed_measurements_sigma + sigma_observation_noise_prev
        hundisturbed_measurements_mean = np.dot(hundisturbed_measurements_sigma, sigma_mean_weights)
        measurements_mean = np.dot(measurements_sigma, sigma_mean_weights)
        # alternately the old code just took the mean states

        # # estimate observation noise
        observation_noise_sample = (Y[k + 1,:].T - hundisturbed_measurements_mean)
        # these indexes are how it was in the code, but should they be prev?
        observation_noise_samples[:, k + 1] = observation_noise_sample
        gamma = np.dot((sigma_mean_weights.T * hundisturbed_measurements_sigma), (hundisturbed_measurements_sigma).T) - np.dot(hundisturbed_measurements_mean, hundisturbed_measurements_mean.T)
        gammas[:,:, k+1] = gamma
        ...
        if k < N0:
            ...
            obs_cov = np.dot((observation_noise_sample - observation_noise_mean), (observation_noise_sample - observation_noise_mean).T)
            old_obs_cov = np.dot((observation_noise_samples[:, k+1-Nr] - observation_noise_mean), (observation_noise_samples[:, k+1-Nr] - observation_noise_mean).T)
            mix_obs_cov = np.dot((observation_noise_sample - observation_noise_samples[:, k+1-Nr]), (observation_noise_sample - observation_noise_samples[:, k+1-Nr]).T / Nr)
            observation_noise_covariance = np.squeeze(observation_noise_covariances[:,:, k])+(obs_cov - old_obs_cov + mix_obs_cov) / (Nr - 1) + (np.squeeze(gammas[:,:, k+1-Nr])-gamma) / Nr
        ...
        measurements_covariance = np.dot((sigma_cov_weights.T * (measurements_sigma - measurements_mean)), (measurements_sigma - measurements_mean).T)
        ...
        covariance_states_measurements = np.dot((sigma_cov_weights.T * (sigma_states_params_new - mean_states_params_new)), (measurements_sigma - measurements_mean).T)

        kalman_gain = covariance_states_measurements / measurements_covariance

                                          adjusted_states_params = mean_states_params_new + kalman_gain * (Y(k+1,:)
        '-measurements_mean)
        ...
        # params(params > para_ub
        ') = para_ub(params > para_ub')
        # params(params < para_lb
        ...
        pie = (sigma_mean_weights
               ' .* fundisturbed_sigma_states_params_new) * fundisturbed_sigma_states_params_new' - fundisturbed_mean_states_params * fundisturbed_mean_states_params' - adjusted_covariance
               pies(:,:, k+1) = pie
        ...
        '
        old_noise_cov = (process_noise_samples(:, k+1-Nq) - process_noise_mean)*(process_noise_samples(
                                                                                :, k+1-Nq) - process_noise_mean].T
        mix_noise_cov = (process_noise_sample - process_noise_samples(:, k+1-Nq))*(process_noise_sample - process_noise_samples(
                                                                                   :, k+1-Nq)].T/Nq
        process_noise_covariance = squeeze(process_noise_covariances(:,:, k))+(noise_cov - old_noise_cov + mix_noise_cov) / (
                    Nq - 1) + (squeeze(pies(:,:, k+1-Nq))-pie) / Nq
        ...
        process_noise_covariance(num_states + 1: end, num_states + 1: end) = process_noise_covariance(
            num_states + 1: end, num_states + 1: end).*factor_matrix
        # process_noise_covariance(num_states + 1: end,:)=process_noise_covariance(num_states + 1: end,:).*kron(
            ones(1, num_states_and_params), factor)
        # process_noise_covariance(:, num_states + 1: end)=process_noise_covariance(:, num_states + 1: end).*kron(
            ones(num_states_and_params, 1), factor
        ')
        # mean
        value
        of
...
function
next_state = predict_next_state(states, control, params, dt) \
             # min_velo = 0.1
lFR = 0.57
...
function
A_hat = ensure_wellformed(A, Amax, Amin)
A_hat = 0.5 * (A + A')
...
function
constrained = satisfy_constraints(x_p, cov, bounds)
num_params = length(bounds) / 2
num_states = length(x_p) - num_params
con_factor = [zeros(num_params, num_states) eye(num_params)
zeros(num_params, num_states) - eye(num_params)]; # let - Vx < 0 -> Vx > 0
