...
class QpSmooth:
    ...
    def drawRaceline(self,lineColor=(0,0,255), img=None):
        rows = 20#self.gridsize[0]
        cols = 20#self.gridsize[1]
        self.resolution = 100
        res = self.resolution
    ...
    def checkTrackBoundary(self,coord,n,delta_max):
        return max(math.sqrt((coord[0] - 5)**2 + (coord[1]-5)**2) - 2, 0), max(math.sqrt((coord[0] - 5)**2 + (coord[1]-5)**2) - 2, 0)
