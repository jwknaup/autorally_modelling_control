...
def get_cur_reg_from_s(s):
    ...
    for ii in range(len(map_params)):
        if s < map_params[ii+1][3]:
            return ii


def plot(states, controls, sim_length, obs):
    # plt.figure()
    plt.cla()
    ...
    obs = obs[:, :, :, 0]
    # obs[1, 0, 0] += 0.25
    reg = get_cur_reg_from_s(obs[0, 1, 0])
    print('reg: ', obs[0,1,0], reg)
    x0 = map_params[reg-1][0]
    y0 = map_params[reg-1][1]
    theta = map_params[reg-1][2]
    s0 = map_params[reg][3]
    rho = map_params[reg][5]
...
def roll_out_trajectory(x_0, us, N, model, dt, lin=None):
    if lin:
        ...
        xs[:, 0] = x_0.flatten()
        for ii in range(N - 1):
            xs[:, ii + 1] = model.update_dynamics(xs[:, ii:ii+1], us[:, ii:ii+1], dt, throttle_nn=model.throttle).flatten()
        # print(x_0, us, xs)
    return xs
...
def run_simple_controller():
    ...
    N = 20
    ar = Model(N)
    x_target = np.tile(np.array([5, 0, 0, 0, 0, 0, 0, 0]).reshape((-1, 1)), (N, 1))
    ...
    D = np.zeros((n, l))
    mu_N = np.array([5., 2., 2., 60., 60., 4., 4., 100.]).reshape((-1, 1))
    ...
    obs = np.array([[[-0.25, 0.25], [4.5, 5.5]], ])
    obs = obs.reshape((1, 2, 2, 1))
    obs = np.tile(obs, (1, 1, 1, N))
    for ii in range(N):
        obs[0, 1, :, ii] += 3*0.1*(ii+1)
    solver = CSSolver(n, m, l, N, v_range, slew_rate, (True, 4*N), mean_only=True, k_form=1, prob_lvl=0.90)
    solver.M.setSolverParam("mioTolAbsGap", 10)
    ...
    try:
        for ii in range(int(sim_length/1)):
            ...
            solver.populate_params(A, B, d, D, xs[:, 0], sigma_0, sigma_N_inv, Q_bar, R_bar, us[:, 0], x_target,
                                   mu_N, 4, K=np.zeros((m * N, n * N)), obs=obs)
            # A = np.eye(8)
            # A[0, 0] = 0.5
            # A[0, 4] = 0.5
            # A[5, 2] = 1
            ...
            try:
                ...
                V = np.tile(np.array([0, -0]).reshape((-1, 1)), (N, 1)).flatten()
                K = np.zeros((m*N, n*N))
            # ks[:, :, ii] = K[:, :]
            # ss[0, ii] = state[7, 0]
            # nearest = np.argmin(np.abs(state[7, 0] - ss[0, :]))
            ...
            print(us[:, 0])
            obs = update_opponent_kinematics(obs, 0.1)
            ...
            # y = xs[:, 0:1].copy()
            xs[:, 0] = state.flatten()
            xs = roll_out_trajectory(xs[:, 0], us, N, ar, 0.1)
            # print(xs)
            ...
            img = plot(states[:, :ii], controls[:, :ii], ii, obs)
            imgs.append(img)

        imageio.mimsave('cs_animation.gif', imgs, fps=10)
