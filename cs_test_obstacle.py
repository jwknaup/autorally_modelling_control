...
def run_simple_controller():
    ...
    obs =np.array([[[-2, 0.5], [25, 26]], [[-0.5, 2], [32, 33]]])
    obs = obs.reshape((2, 2, 2, 1))
    obs = np.tile(obs, (1, 1, 1, N))
    solver = CSSolver(n, m, l, N, v_range, slew_rate, (True, 8*N), mean_only=True, k_form=1, prob_lvl=0.90)
    ...
    try:
        ...
        obs = obs.reshape((2, 2, 2, N))[:, :, :, 0]
        # obs[1, 0, 0] += 0.25
        x0 = map_params[1][0]
        y0 = map_params[1][1]
        theta = map_params[1][2]
        s0 = map_params[2][3]
